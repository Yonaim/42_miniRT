...
static int	parse_element_info(t_ray_tracing *rt, char *str, int elem_exist[])
{
	...
	if ((elem_type == AMBIENT || elem_type == CAMERA || elem_type == LIGHT) \
		...
		return (FAILURE);
	if (parse_func[elem_type](rt, str) == FAILURE)
		return (FAILURE);
...
int	parse_scene(t_ray_tracing *rt, char *path)
{
	...
	while (line)
	{
		if (is_empty_string(line))
			continue ;
		if (parse_element_info(rt, line, elem_exist) == FAILURE)
			return (FAILURE);
		line = get_next_line(fd);
		printf("parsed...\n");
	}
