...
int	build_element_point_light(
	t_world *world, t_point3 *pos, double *brightness, t_vector3 *rgb)
{
	...
	ptl_info.rgb = *rgb;
	if (add_object_to_world(world, new_point_light(&ptl_info)) == FAILURE)
		return (FAILURE);
	return (SUCCESS);
}

int	build_element_plane(
	t_world *world, t_point3 *pos, t_vector3 *normal, t_vector3 *rgb)
{
	...
	if (add_object_to_world(world, new_plane(&pl_info)) == FAILURE)
		return (FAILURE);
	return (SUCCESS);
}

int	build_element_sphere(
	t_world *world, t_point3 *pos, double *diameter, t_vector3 *rgb)
{
	...
	if (add_object_to_world(world, new_sphere(&sp_info)) == FAILURE)
		return (FAILURE);
	return (SUCCESS);
}
